...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($login_type === 'phone') {
            ...
            if (empty($phone)) {
                ...
                if (isset($_SESSION['phone_verification_code']) && 
                    ...
                    if ($user) {
                        ...
                        $redirectUrl = BASE_URL . ($user['test_completed'] ? 'results/view.php' : 'dashboard/index.php');
                        if ($isAjax) {
                            header('Content-Type: application/json');
            ...
            if ($user) {
                $_SESSION['user_id'] = $user['id'];
                redirect(BASE_URL . ($user['test_completed'] ? 'results/view.php' : 'dashboard/index.php'));
            } else {
                $error = 'Foydalanuvchi topilmadi!';
            }
